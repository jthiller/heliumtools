...
- Tools needing Solana wallets are wrapped in `SolanaProvider` in `main.jsx` (`src/multi-gateway/SolanaProvider.jsx`), whose RPC endpoint comes from the build-time `VITE_SOLANA_URL`, falling back to the public Solana RPC with a console warning when unset
- **Always keep `pages/public/.env.example` in sync** — when you add, rename, or remove any `VITE_*` variable the frontend reads, update it in the same change. It is the committed, placeholder-only list (the `.env*` files themselves are gitignored). **Nothing in it is secret:** Vite inlines every `VITE_*` value into the client bundle, so an API key in one of these URLs ships publicly and must be restricted by origin at the provider rather than treated as hidden
...
- Other shared libs in `src/lib/`: `jupiter.js` (Jupiter Price v3 client on the keyed `api.jup.ag` host, `JUPITER_API_KEY` when set with a keyless fallback — the only token-USD source, shared by `hnt-price` and `wallet-dashboard`), `kv.js` (best-effort KV JSON helpers + `withKvLock`), `rateLimit.js`, `response.js`, `solanaRpc.js`
