...
- Shared Helium × Solana library: `src/lib/helium-solana.js` (program IDs, PDAs, instruction builders) — used by `multi-gateway`, and by `dc-mint` / `dc-purchase` / `hnt-price` for `resolveHntPriceOracle()` (the one implementation of the DataCreditsV0 oracle read)
