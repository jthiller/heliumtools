...
| HNT Price | `worker/src/tools/hnt-price/CLAUDE.md` | **Worker-only, no frontend** — public HNT price API for other teams (replaces unauthenticated Pyth Hermes): KV snapshot (`/current`), live chain read (`/instant`), and two streams off the same `HntPriceHub` DO — WebSocket (`/ws`) and SSE (`/sse`), sharing one subscriber ceiling. `README.md` next to it is the external API reference |
